...
# from https://github.com/tu-curious/Bayes_by_Backprop-TF/blob/master/BBB_MNIST
#other websites to check:
#https://gist.github.com/vvanirudh/9e30b2f908e801da1bd789f4ce3e7aac
#https://github.com/TENGBINN/Bayes-by-Backprop/blob/master/bbb_training.py


"""
Created on 09-24
Modifying to get two layers only
"""
...
config = {
    "num_hidden_layers": 1, # was 2
    "num_hidden_units": 64, # was 400
...
with tf.name_scope('params_final_layer'):
    n_output = 10
    W2_mu = tf.Variable(tf.truncated_normal([n_hidden_1, n_output],mean=0., stddev=stddev_var),name='W2_mu')
    W2_rho = tf.Variable(tf.constant(rho_prior,shape=[n_hidden_1, n_output]),name='W2_rho') 
    b2_mu = tf.Variable(tf.truncated_normal([n_output],mean=0., stddev=stddev_var),name='b2_mu')
    b2_rho = tf.Variable(tf.constant(rho_prior,shape=[n_output]),name='b2_rho')
...
for _ in range(n_samples):
    ...
    epsilon_w2 = get_random((n_hidden_1, n_output), avg=0., std=1.)
    epsilon_b2 = get_random((n_output,), avg=0., std=1.)
...
with tf.name_scope('Prediction_network'):
    ...
    y_hat_mu = tf.nn.softmax((tf.matmul(a1_mu, W2_mu) + b2_mu))
...
scope_list=['params_layer_1','params_final_layer']
